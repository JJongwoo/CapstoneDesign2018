...
const int sonaPin_InnerroomBathroom = 3;  // 초음파 센서 (안방-화장실)
const int sonaPin_InnerroomOutside =  4;  // 초음파 센서 (현관-안방)
const int shockPin =                  5;  // 충격감지 센서 (안방)
const int buttonPin =                 6;  // SOS 버튼 (안방)
...
bool IsDetectPersonInnerroomBathroom() {
  detectValue = digitalRead(sonaPin_InnerroomBathroom);
...
bool IsDetectPersonInnerroomOutside() {
  detectValue = digitalRead(sonaPin_InnerroomOutside);
  if(detectValue <= sonaStandard)
    return true;
  else
    return false;
...
void setup() {
  ...
  pinMode(sonaPin_InnerroomBathroom, INPUT);
  pinMode(sonaPin_InnerroomOutside, INPUT);
