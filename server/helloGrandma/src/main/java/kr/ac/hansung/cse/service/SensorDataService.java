...
public class SensorDataService {
	...
	public void saveData(String pir1, String pir2, String id, String sona1, String sona2, String btn) {
		...
		// 화장실 초음파가 줄어들고 화장실 적외선이 1이고. 거실 적외선 0이면 체크.
	      ...
	      if(Integer.parseInt(sona2) >= 12 && pir1.equals("1") && pir2.equals("0")) {
	         if(bath_count<5) {// 몇 초 이하는 화장실 이용 체크 안할지 정해야함.
	            bath_count=0;
	         }
	         else if(bath_count>15) { // 몇 초 이상 화장실에 앉아 있을 경우 화장실을 이용했는지 정해야함.
	            // DB 상의 화장실 이용횟수 증가
	            System.out.println("화장실 이용");
	            emergency = new Emergency();
	            emergency.setEmergency_id(id);
	            emergency.setTime(time);
		...
		// sensorDataDao.saveData(pir1, pir2, date, time, id, sona1, sona2, btn, "실내");
		...
		sensorDataDao.saveData(pir1, pir2, date, time, id, sona1, sona2, btn, location);
